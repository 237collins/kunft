...
class _VillasState extends State<Villas> {
  ...
  void initState() {
    super.initState();
    _fetchVillas(); // ✅ Appelle la fonction de fetch locale
  }

  // ✅ Fonction de fetch locale pour les villas
  Future<void> _fetchVillas() async {
    ...
    try {
      ...
      final data = await Provider.of<LogementProvider>(
        context,
        listen: false,
      ).fetchLogementsForType('Villa', token);
      setState(() {
        _logements = data;
        _isLoading = false;
      });
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = 'Impossible de charger les villas : $e';
      });
      if (mounted) {
      ...
      builder: (context, logementProvider, child) {
        ...
        return Scaffold(
          body: isLoading
              ...
              : logements.isEmpty
              ? WidgetEmptyHouse(
                  message: 'Aucune Villa disponible pour le moment.',
                )
              : Padding(
