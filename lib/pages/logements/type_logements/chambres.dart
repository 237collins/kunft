...
class _ChambresState extends State<Chambres> {
  ...
  void initState() {
    super.initState();
    _fetchChambres(); // ✅ Appelle la fonction de fetch locale
  }

  // ✅ Fonction de fetch locale pour les chambres
  Future<void> _fetchChambres() async {
    ...
    try {
      ...
      final data = await Provider.of<LogementProvider>(
        context,
        listen: false,
      ).fetchLogementsForType('Chambre', token);
      setState(() {
        _logements = data;
        _isLoading = false;
      });
    } catch (e) {
      print('DEBUG: Erreur lors du chargement des chambres: $e');
      setState(() {
        _isLoading = false;
        _errorMessage = 'Impossible de charger les chambres : $e';
      });
      if (mounted) {
